...
为了设置 Doris Extract 节点, 下面提供了使用构建自动化工具（例如 Maven 或 SBT）和带有 Sort Connector JAR 包的 SQL 客户端的两个项目的依赖关系信息。
...
| fenodes | 可选 | (none)            | string  | Doris FE http 地址                                                                             |
| table.identifier	 | 可选 | (none)            | string  | Doris 表名，如：db1.tbl1                                                                          |
...
请参阅 [格式](https://github.com/apache/doris/blob/1.0.0-rc03/docs/zh-CN/extending-doris/flink-doris-connector.md) 页面以获取更多细节。